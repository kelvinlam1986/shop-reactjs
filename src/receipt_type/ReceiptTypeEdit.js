...
class ReceiptTypeEdit extends Component {
    render() {
        const {
            handleSubmit,
            isShowModal,
        ...
        return (
            <RegisterModal
                ...
                hiddenFooter={false}
                showCancel={true}
                clickOK={handleSubmit(updateReceiptType)}
                okText="Lưu"
                pristine={pristine}
...
const mapStateToProps = state => {
    const { currentReceiptType } = state.receiptType;
    return {
        initialValues: currentReceiptType
    };
};

const rxForm = reduxForm({
    form: "ReceiptTypeEditPage",
